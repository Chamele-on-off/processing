...
def get_current_trader():
    ...
    return jsonify({
        'id': user['id'],
        'name': user.get('name', 'Trader User'),
        'email': user['email'],
        'working_balance_usdt': float(user.get('working_balance_usdt', 0)),
        'working_balance_rub': float(user.get('working_balance_rub', 0)),
        'deposit_rate': float(user.get('deposit_rate', 0.01)),
        'withdrawal_rate': float(user.get('withdrawal_rate', 0.01)),
        'deposits_enabled': bool(user.get('deposits_enabled', True)),
        'withdrawals_enabled': bool(user.get('withdrawals_enabled', True))
    })

@app.route('/api/trader/transactions')
@login_required
@role_required('trader')
def get_trader_transactions():
    ...
    # Add merchant emails to transactions
    result = []
    for t in transactions:
        merchant = db.find_one('users', {'id': t['merchant_id']}) if 'merchant_id' in t else None
        transaction_data = {
            'id': t['id'],
            'type': t['type'],
            ...
            'receipt_file': t.get('receipt_file')
        }
        result.append(transaction_data)
    
    return jsonify(result)

@app.route('/api/trader/requisites')
@login_required
@role_required('trader')
def get_trader_requisites():
    user = get_current_user()
    requisites = db.find('requisites', {'trader_id': user['id']})
...
def create_trader_requisites():
    ...
    try:
        ...
        requisite = db.insert_one('requisites', requisite_data)
        return jsonify(requisite)
    except Exception as e:
        return jsonify({'error': str(e)}), 400

@app.route('/api/trader/requisites/<int:requisite_id>', methods=['PUT'])
@login_required
@role_required('trader')
def update_trader_requisites(requisite_id):
    user = get_current_user()
    data = request.get_json()
    ...
    # Verify the requisite belongs to the current trader
    requisite = db.find_one('requisites', {'id': requisite_id, 'trader_id': user['id']})
    if not requisite:
        return jsonify({'error': 'Requisite not found or access denied'}), 404
    ...
    try:
        ...
        db.update_one('requisites', {'id': requisite_id}, updates)
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
...
def upload_transaction_receipt(transaction_id):
    ...
    # Verify the transaction belongs to the current trader
